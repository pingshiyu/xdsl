...
class Split(Operation):
    ...
    # attributes
    w1_qubits = AttributeDef(BaseAttr(IntAttr)) # qubits in wire 1. TODO: decide if this should be operand or attr?
    ...
    @staticmethod
    def apply(w_in: Union[Operation, SSAValue], w1_qubits: Union[int, Attribute]) -> Split:
        ...
        if w1_n_qubits <= 0 or w2_n_qubits <= 0:
            ...
            raise DiagnosticException(f"Cannot split type {w_in_n_qubits} wire into type {(w1_n_qubits, )} wires")

        w1_type = Qubits([IntAttr(w1_n_qubits)])
        w2_type = Qubits([IntAttr(w2_n_qubits)])
        return Split.build(operands=[w_in_ssa], result_types=[w1_type, w2_type], attributes={"w1_qubits": w1_qubits})
...
class Pauli(Operation):
    ...
    @staticmethod
    def apply(
        input_qubits: Union[Operation, SSAValue], pauli_type: str
    ) -> Pauli:
        return Pauli.build(
